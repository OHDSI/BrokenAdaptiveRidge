  ...
  if (old) {
    ...
    cyclopsFit <- fitCyclopsModel(cyclopsData,
                                  prior = createBarPrior(penalty = lambda,
                                                         exclude = c(0),
                                                         initialRidgeVariance = 1 / lambda_r))
...
if (old) {
  write.csv(all_results,file="C:\\Cyclops\\simulation_n70kp20k_old.csv")
} else {
  write.csv(all_results,file="C:\\Cyclops\\simulation_n70kp20k_new.csv")
}
